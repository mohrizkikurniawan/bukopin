...
* <Rating>-181</Rating>
*-----------------------------------------------------------------------------
    SUBROUTINE ATI.VID.USER.ATTEMPT
*-----------------------------------------------------------------------------
* Developer Name     : Dwi K
* Development Date   : 20171020
* Description        : Routine for check id reset users
*-----------------------------------------------------------------------------
...
*-----------------------------------------------------------------------------
    ...
    IF COMI AND (V$FUNCTION = 'I') THEN
        COMI = "A-":Y.DATE.VAL[6]:Y.TIME
    END
	
    Y.USER.ID = System.getVariable('CURRENT.USER.ID')

    SEL.CMD = "SELECT F.PASSWORD.RESET$NAU WITH USER.ATTEMPT EQ ": Y.USER.ID
    ...
    IF SEL.LIST THEN
        ...
        CALL ERR
    END ELSE
        R.NEW(EB.PWR.USER.ATTEMPT) = Y.USER.ID
    END
